...
void *connection_handler(void *threadid){
	...
	// int tid;
    struct SEND *sendstruct = malloc(sizeof(struct SEND));

	// tid = pthread_getthreadid_np();
	printf("Connected succesfully client: %ld\n", pthread_self());
	int i = 1;
	while(i <= 20){
        sendstruct->data = serv_addr;
        sendstruct->num = i;
        void* myptr = &sendstruct;
		printf("For thread : \t%ld Send data %d\n", pthread_self(), sendstruct->num);
